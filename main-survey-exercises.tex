...
\end{question}
\begin{solution}
As described in \cite[Proposition~A.16]{AcharyaCT19b}, the reduction does go through, as each user can ``locally'' and independently apply the mapping $\Psi_\q$ to their sample, to get a draw from the distribution $\Phi_\q(\p)$. This preserves the setting of randomness (private- or public-coin), as it only require private randomness; however, as mentioned in~\cite[Remark~A.17]{AcharyaCT19b}, all users \emph{do} need to know the reference distribution $\q$ (in order to be able to compute the mapping~$\Psi_\q$.
