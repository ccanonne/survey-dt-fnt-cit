...
\[
  ...
  \leq \frac{5\ab}{4\dst^4\ns_1\ns_2} + \frac{\ns_1+\ns_2}{\ns_1\ns_2}\Paren{\frac{2\sqrt{\ab}}{\dst}+\frac{3}{\dst^2}}\,.
\]
At first glance, this looks perfect: our tester will be correct in both cases as long as $\ns_1\ns_2 \gtrsim \ab/\dst^4$, which is what we want, and $\frac{\ns_1\ns_2}{\ns_1+\ns_2} \gtrsim \max\big(\frac{\sqrt{\ab}}{\dst}, \frac{1}{\dst^2}\big)$. However, that last condition is an issue: if we wanted to set $\ns_1\gg \ns_2$, for instance, then we would still need
\[
    \ns_2 \gtrsim \max\Paren{\frac{\sqrt{\ab}}{\dst}, \frac{1}{\dst^2} }\,,
\]
which, given that the ``vanilla'' collision-based tester corresponded to $\ns_1=\ns_2 \asymp \frac{\sqrt{\ab}}{\dst^2}$, is not much of a tradeoff between $\ns_1$ and $\ns_2$ at all\dots
...
\begin{align}
  \Var_\p[Z_6]
  &\leq \frac{1}{\ns_1\ns_2}\cdot \frac{1+\alpha^2}{\ab} + \frac{\ns_1+\ns_2}{\ns_1\ns_2}\cdot \frac{7\alpha^2}{\ab^2} \label{eq:var:bipartite:collisions:lowinfty}\,.
\end{align}
...
\begin{theorem}
  ...
  Let $f\colon \R^\ab\to\R$ by a symmetric convex function. For a probability distribution $\p\in\distribs{\ab}$ and $\ns_1,\ns_2\in\N$, define the random variable $X_{\ns_1,\ns_2}(\p)$ as follows: let $N_1,\dots,N_\ab$ and $M_1,\dots,M_\ab$ be the counts of each domain element among $\ns_1$ and $\ns_2$ \iid samples from $\p$, respectively, and set $X_{\ns_1,\ns_2}(\p)=f(N_1M_1,\dots,N_\ab M_\ab)$. Then, for any $\p,\q$ such that $\p\succeq\q$, $X_{\ns_1,\ns_2}(\p)$ stochastically dominates $X_{\ns_1,\ns_2}(\q)$.
\end{theorem}
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        &\leq \frac{\ab}{\ns_1\ns_2}\cdot \frac{4(1+\alpha^2)}{\alpha^4} + \frac{\ns_1+\ns_2}{\ns_1\ns_2}\cdot \frac{28}{\alpha^2}  \\
        &\leq \frac{8\ab}{\ns_1\ns_2\dst^4} + \frac{56}{\min(\ns_1,\ns_2)\dst^2}
    \end{align*}
    using~\cref{eq:var:bipartite:collisions:lowinfty}, then $\alpha^2\geq \dst^2$ (and $\dst \leq 1$) and $\ns_1+\ns_2\leq 2\max(\ns_1,\ns_2)$. This is less than $1/6+1/6=1/3$ as long as (1)~$\ns_1\ns_2 \geq 24\ab/\dst^4$ and~(2) $\min(\ns_1,\ns_2) \geq 168/\dst^2$.
  \end{itemize}
  The above analysis shows that both errors are less than $1/3$ for any $\ns = \ns_1+\ns_2$ such that $\ns_1\ns_2 \geq \lceil{24\ab/\dst^4}\rceil$ and $\min(\ns_1,\ns_2) \geq \clg{168/\dst^2}$; this concludes the proof.
\end{proof}
