...
Now we turn to a tester that is \emph{not} sample-optimal~--~but has other advantages, and whose analysis contains a couple of insighful aspects. The main idea is that, while large domains are complicated, if there is one thing we know how to do optimally it is estimating the bias of a coin. That is, we know how to handle the case $\ab=2$:
...
\]
samples by~\cref{lemma:bias:coin}. Of course, there is a catch: we will only detect it when there \emph{is} some bias to detect, \ie when the random set $S$ we choose is ``good;'' and we only proved this happens with probability at least $1/48$. But a constant probability to distinguish between uniform and far from uniform is enough: independently repeating the test several times will let us amplify the success probability to $2/3$.
\begin{theorem}
...
Applying~\cref{theo:l2:identity:robust} with $\normtwo{\q} = 1/\sqrt{\ab}$ and distance paraneter $\dst' \eqdef \dst/\sqrt{\ab}$ (from the relation TV/$\lp[2]$,~\cref{eq:relation:l1:l2:cs}), we can distinguish between
...
\begin{algorithm}
  ...
  \Ensure Returns \textsf{accept} if $\lVert p -q\rVert_2 \leq \eps$ and \textsf{reject} if $\lVert p -q\rVert_2 \geq 2\eps$.
...
% \begin{proposition}\label{prop:ggg:2}
% There exists an absolute constant $c > 0$ such that the above algorithm, when given $\Poi(m)$ samples drawn from two distributions, $p,q$ will, with probability at least $3/4$, 
% distinguishes between \textsf{(a)} $\lVert p-q\rVert_2 \leq \eps$ and \textsf{(b)} $\lVert p-q\rVert_2 \geq 2\eps$ provided that $m \geq c\frac{\sqrt{b}}{\eps^2}$, where $b$ is an upper bound on $\lVert p\rVert_2^2, \lVert q\rVert_2^2$. (Moreover, one can take $c = 130$.)
% \end{proposition}
