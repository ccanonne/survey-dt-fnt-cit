...
\tbc
% To prove this last statement, observe that the case $m=n=1$ can be shown from the identity $MN+(1-M)(1-N) = \indic{M=N}$, and the fact that $\bPr{M=N}=p^2+(1-p)^2 \geq q^2+(1-q)^2 = \bPr{M'=N'}$. The general case $m,n\geq 1$ then follows from rewriting the binomials as sum of \iid Bernoullis $M=\sum_{\ell=1}^m X_i$, $N=\sum_{\ell=1}^n Y_i$, so that
% \[
%     MN+(m-M)(n-N) = \sum_{\ell=1}^m\sum_{\ell'=1}^n ( X_\ell Y_{\ell'} + (1-X_\ell)(1-Y_{\ell'}) )\,;
% \]
% and use the case $m=n=1$ along with the fact that addition of independent summands preserves stochastic dominance~\citep{MathSE-massai}.
\end{proof}
