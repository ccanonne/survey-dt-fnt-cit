...
Corollary where all $\alpha_i$'s are the same: $\Omega(1/(\dst^2\normtwo{\q}))$.
Application:
\begin{itemize}
	\item uniformity: $\alpha_i=\dst$ for all $i$. Gives optimal bound, since $\normtwo{\q}=1/\sqrt{\ab}$.
	\item Binomial: same. Gives optimal bound, since $\normtwo{\q}\asymp 1/\ab^{1/4}$.
\end{itemize}
Now, an example where the corollary is not tight:

Consider $\q$ on $[\ab]$ such that
\[
	\q(i) = \frac{1}{\Lambda(\ab) \sqrt{i}}
\]
where
\[
	\Lambda(\ab) \eqdef \sum_{i=1}^\ab \frac{1}{\sqrt{i}} \operatorname*{\sim}_{\ab\to\infty} 2\sqrt{\ab}
\]
It is not hard to check that
\[
	\normtwo{\q} = \frac{\sqrt{H_\ab}}{\Lambda(\ab)} \operatorname*{\sim}_{\ab\to\infty} \frac{\ln \ab}{2\sqrt{\ab}}
\]
so the corollary gives a lower bound of $\bigOmega{\frac{\sqrt{\ab}}{\dst^2\sqrt{\log \ab}}}$. But now, let us apply the theorem itself, with
\[
	\alpha_i \eqdef 
	\begin{cases}
		4\dst &\text{if } i \geq \frac{\ab}{2}\\
		0 &\text{ otherwise}
	\end{cases}
\]
(we perturb only the tail!) 
Then, setting $L\eqdef \clg{\frac{\ab}{2}}$,
\[
	\sum_{i=1}^\ab \alpha_i^4 \q(i)^2 = \frac{256\dst^4}{\Lambda(\ab)^2}\sum_{i=L}^\ab \frac{1}{i} \asymp \frac{\dst^4}{\ab}
\]
which gives a lower bound of $\bigOmega{\frac{\sqrt{\ab}}{\dst^2}}$. 
since $\sum_{i=L}^\ab \frac{1}{i}\asymp 1$. Moreover, we do have the distance we want, as
\[
	\sum_{i=1}^\ab \alpha_i\q(i) = 	\frac{4\dst}{\Lambda(\ab)} \sum_{i=L}^\ab \frac{1}{\sqrt{i}} \asymp \dst
\]
while $\max_i \alpha_i\q(i) = 4\dst \q(L) \ll \dst$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
