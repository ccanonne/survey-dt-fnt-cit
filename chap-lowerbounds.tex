...
\begin{equation}
	...
	\leq \frac{2^j\ns^j}{\ab^{j-1}}
\end{equation}
Moreover, for the special case of $j=2$, the expression is a little nicer, and becomes
\begin{equation}
	m^{\no}(2) = \ns^2\Paren{ \frac{\ab}{2} \Paren{\frac{1+3\dst}{\ab}}^2 + \frac{\ab}{2} \Paren{\frac{1-3\dst}{\ab}}^2 }
	= (1+9\dst^2)\frac{\ns^2}{\ab}\,.
\end{equation}
Why do we handle separately the second $\ns$-based moment? Without wanting to spoil the surprise, it's because we expect the term $j=2$ of the series $\sum_{j=2}^\infty \frac{\abs{m^{\yes}(j)-m^{\no}(j)}}{\sqrt{1+\max(m^{\yes}(j),m^{\no}(j))}}$ to dominate (as the second moment $\normtwo{\p}^2$ of the distribution is ``what gives it away'' in uniformity testing, as we saw now and again in~\cref{sec:uniformity}), so we want to make sure we handle that term as tightly as possible.

With the above expressions at our disposal, we can proceed: first, this maximum in the denominator seems annoying and will prevent us from easily computing the series, so let's get rid of it:
\begin{align*}
\sum_{j=2}^\infty \frac{\abs{m^{\yes}(j)-m^{\no}(j)}}{\sqrt{1+\max(m^{\yes}(j),m^{\no}(j))}}
&\leq
...
\noindent What went wrong? We were a little too eager to get rid of ``this maximum in the denominator.'' It \emph{is} annoying, and it \emph{is} a good idea to get rid of it in order to be left with a geometric series, but \emph{not} in that first term we handle separately. There, we need it\dots{} So let's try again.
\begin{align*}
\sum_{j=2}^\infty \frac{\abs{m^{\yes}(j)-m^{\no}(j)}}{\sqrt{1+\max(m^{\yes}(j),m^{\no}(j))}}
&\leq
...
\begin{theorem}
  \label{theo:uniformity:lb:valiant}
...
\end{theorem}
The key aspect of this lower bound was how \emph{painless} it was to obtain it. The main idea was to use the same Paninski construction as before, check a couple conditions, compute a geometric series after handling separately the first (dominating) term, and then conclude by~\cref{theo:valiant:wishful}. 

To conclude, let's mention that the restriction on $\dst$ in~\cref{theo:uniformity:lb:valiant} can be relaxed a little to $\dst\geq 1/\ab^{1/8}$, by being a little more careful about bounding the $\ns$-based moments (especially for $j=3$)~\exercise{Try it!}. However, it can only get us so far, as the first condition of the theorem, $\norminf{\p^\yes} \leq \frac{1}{500\ns}$, by itself imposes $\ns \lesssim \ab$, leading to $\dst\gtrsim 1/\ab^{1/4}$.
